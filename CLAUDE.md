...
- **화면은 Figma가 1순위다.** Figma 수치가 어색해 보여도(예: 옆 열보다 20px 짧은 입력 묶음) "디자인 실수 같다"며 다른 값을 추천하거나 제안하지 않는다. Figma대로 구현하고, 어색한 점은 "Figma 값이 이렇다"는 사실로만 알린다.
- Figma 값은 Figma가 준 형태 그대로 옮긴다. Figma가 `width: 672`면 코드도 폭 672로 쓰고, 결과만 같은 다른 표현(예: "열 폭 − 20"을 `margin-right: 20`으로)으로 바꾸지 않는다.
...
- **한 번에 질문한다**: 아래를 모아 한 번에 확인받는다. 작업 중간에 나눠서 여러 번 묻지 않는다.
  ...
  - Figma에 있는 값을 따를지 말지는 묻지 않는다(위 "화면은 Figma가 1순위다").
...
- Figma 수치 요약은 Figma 속성 그대로 넘긴다(폭은 width로. "열 폭 − 20" 같은 파생 표현으로 바꾸지 않는다). 사용자가 승인하지 않은 Figma와의 차이(예: 한 카드만 다른 간격을 나머지와 통일)는 구현 지시에 넣지 않는다.
...
- 승인된 1~4단계대로 구현하고 G1부터 차례로 돌린다. 실패해서 코드를 고쳤으면 G1부터 다시 돌린다(G4에서 SCSS를 고쳤어도 G2 lint·G3 `reviewer`를 건너뛰지 않는다). 원인을 고쳐서 예전에 "파생 문제"로 제외했던 항목도 다시 확인한다.
