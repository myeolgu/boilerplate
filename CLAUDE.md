...
- 위 컴포넌트(체크박스·라디오·셀렉트·탭·아코디언·모달·스와이퍼·텍스트영역·날짜 선택 등 인터랙션이 있는 것들)는 전부 `initUI()`가 해당 `.component-*` 클래스를 스캔해 자동 초기화하는 실제 JS 컴포넌트(`src/assets/scripts/ui/components`)와 연결되어 있으므로 클래스와 `data-props-*` 속성을 임의로 바꾸지 않는다.
