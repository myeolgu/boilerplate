...
- **빌드 성공 여부 확인은 `npm run build`(= `gulp build --build`, 한 번 실행하고 종료)로 한다.** `npm run dev`는 `gulp.watch`로 파일을 계속 감시하는 워처 + 로컬 서버(8888)라 **절대 스스로 끝나지 않는다.** 이걸 "빌드가 끝나길" 기다리면 그대로 멈춘다. dev 서버는 Playwright로 화면을 열어야 할 때만 띄우고, 띄운 뒤에는 종료를 기다리지 말고 다음 작업으로 넘어간다.
