...
1. **대상 node 확인**: 사용자가 준 Figma 링크에 `node-id`가 있는지 확인한다. 없으면 임의로
   다른 화면을 기준 삼지 않고 사용자에게 확인한다(`layout-page`, CLAUDE.md 원칙과 동일).
2. **`get_design_context` 호출**: Figma MCP의 `figma-design-to-code` 스킬 절차대로 대상 node에
   호출한다. `get_metadata`/`get_screenshot`는 node를 고르거나 결과를 눈으로 검증할 때만
   보조로 쓰고, `get_design_context`를 대체하지 않는다.
...
- 구현 후 CLAUDE.md의 검증 절차(`npm run checkhtml`/`checkstyle`, 컴포넌트 가이드와 마크업 비교,
  필요하면 Playwright로 실제 동작 확인)를 그대로 따른다.
