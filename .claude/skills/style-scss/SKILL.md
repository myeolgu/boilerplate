...
- JS로 켜고 끄는 상태·토글 클래스는 `is-`, `has-` 접두어를 쓰고 기본 클래스에 중첩해 결합한다(`&.is-active`, `&.is-open`).
...
- `_mixins.scss`에 이미 `f12`/`f14`/`f16`/`f18`/`f20`/`f40`/`f56` 폰트 크기 믹스인이 있다. 필요한 크기가 없으면 같은 패턴으로 새 믹스인을 추가한다. `font-size`를 믹스인 없이 직접 쓰지 않는다.
